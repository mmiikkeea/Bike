...
echo "<div class=odSubTitle><div class=odTitle>修改未付款訂單</div>".$_SESSION["account"] . "  共有 ". $total_records." 筆訂單記錄</div>";
?>
<div class=odContent>
...
<tr "<? echo $color ?>">
  ...
  <form action="order_deleteOrder.php" method="post">
    <input type="hidden" name="order_no" 
          value="<?echo $row["order_no"]?>">
   <td align=center><input type="submit" name="deleteOrder" value="刪除"></td>
...
if ( $pages > 1 )  // 顯示上一頁
   echo "<a href='order_modifyOrder.php?Pages=".($pages-1).
        "' class=\"styleP\">上一頁</a>| ";
for ( $i = 1; $i <= $total_pages; $i++ )
   if ($i != $pages)
     echo "<a href=\"order_modifyOrder.php?Pages=".$i."\" class=\"styleP\">".
          $i."</a> ";
   else
     echo $i." ";
