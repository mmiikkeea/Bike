...
// 設定SQL查詢字串
$sql = "SELECT * FROM bikeorder, member where bikeorder.order_memberID=member.member_Id and order_status='確認匯款中' order by order_date desc";  // 建立SQL指令字串
//echo $sql;
// 送出utf8編碼的MySQL指令
...
echo "<div class=odSubTitle><div class=odTitle>確認匯款中訂單</div> </div>";
...
<tr style="border: 1px solid rgb(255, 255, 255); background: rgba(100,150,255,0.80);" cellpadding="5" cellspacing="5" frame="border" rules="all">
   ...
   <th>訂購總金額</th><th>訂單狀態</th><th>匯款帳號</th><th>確認付款</th>
</tr> 
<?
$j = 1;
...
<tr "<? echo $color ?>">   
   ...
   <?
         echo " <form action=\"order_adminBankAcc.php\" method=\"post\">
    <input type=\"hidden\" name=\"order_no\" 
          value=". $row["order_no"] .">
   <td align=center><input type=\"submit\" name=\"sendIsPay\" value=\"確認付款\"></td></tr>
   </form>"; 
