...
echo "<div class=odSubTitle><div class=odTitle>新增訂單成功</div></div>";
?>
<div class=odContent>
...
<table style="border: 1px solid rgb(255, 255, 255); background: rgba(255,255,255,0.65); width: 800px;" cellpadding="5" cellspacing="5" frame="border" rules="none">
  ...
  <tr height=30>
	...
	<form action="order_handleBankAcc.php" method="post">
	 ...
	 /*
	 if ($orderStatus=="未付款")
	   echo "輸入匯款帳號末五碼：<input type=\"text\" size=\"10\" name=\"order_bankAcc\"><input type=\"submit\" name=\"Send\" value=\"輸入\">";
	 
     if ($orderStatus=="確認匯款中")
	   echo "修改匯款帳號末五碼：<input type=\"text\" size=\"10\" name=\"order_bankAcc\"><input type=\"submit\" name=\"Send\" value=\"修改\">";	 
	*/   
...
if ( $pages > 1 )  // 顯示上一頁
   echo "<a href='order_showCreateOrderDetail.php?Pages=".($pages-1).
        "&odrNO=".$orderNO."' class=\"styleP\">上一頁</a>| ";
for ( $i = 1; $i <= $total_pages; $i++ )
   if ($i != $pages)
     echo "<a href=\"order_showCreateOrderDetail.php?Pages=".$i."&odrNO=".$orderNO."\" class=\"styleP\">".
          $i."</a> ";
   else
     echo $i." ";
