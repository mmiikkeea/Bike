...
// 設定SQL查詢字串
$sql = "SELECT * FROM bikeorder, member where bikeorder.order_memberID=member.member_Id order by order_date desc";  // 建立SQL指令字串
//echo $sql;
// 送出utf8編碼的MySQL指令
...
echo "<div class=odSubTitle><div class=odTitle>管理訂單</div> </div>";
...
未付款訂單數:<?echo " ".$total_unpaid ?><br><br>
確認匯款中訂單數:<?echo " ".$total_checking ?><br><br>
處理中訂單數:<?echo " ".$total_processing ?><br><br>
已出貨訂單數:<?echo " ".$total_shipped ?><br><br>
共有<?echo " ".$total_records." " ?>筆訂單記錄<br>
...
<form action="order_modifyOrderDetail.php" method="post">
...
<tr "<? echo $color ?>">   
   ...
   <td align=center><input type="submit" name="Send" value="修改訂單明細"></td>
...
if ( $pages > 1 )  // 顯示上一頁
   echo "<a href='order_adminOrder.php?Pages=".($pages-1).
        "' class=\"styleP\">上一頁</a>| ";
for ( $i = 1; $i <= $total_pages; $i++ )
   if ($i != $pages)
     echo "<a href=\"order_adminOrder.php?Pages=".$i."\" class=\"styleP\">".
          $i."</a> ";
   else
     echo $i." ";
