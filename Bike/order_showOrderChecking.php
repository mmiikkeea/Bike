...
// 設定SQL查詢字串
$sql = "SELECT * FROM bikeorder where order_memberID = '".$_SESSION['id']."' and order_status='確認匯款中' order by order_date desc";  // 建立SQL指令字串
//echo $sql;
// 送出Big5編碼的MySQL指令
...
echo "<div class=odSubTitle><div class=odTitle>確認匯款中訂單</div>".$_SESSION["account"] . "  共有 ". $total_records." 筆訂單記錄</div>";
?>
<div class=odContent>
...
if ( $pages > 1 )  // 顯示上一頁
   echo "<a href='order_showOrderChecking.php?Pages=".($pages-1).
        "' class=\"styleP\">上一頁</a>| ";
for ( $i = 1; $i <= $total_pages; $i++ )
   if ($i != $pages)
     echo "<a href=\"order_showOrderChecking.php?Pages=".$i."\" class=\"styleP\">".
          $i."</a> ";
   else
     echo $i." ";
